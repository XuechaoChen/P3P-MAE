...
from datasets.custom_folder import info_collate_fn
import utils.lr_decay as lrd

def dataset_builder_with_collate_fn(args, config):
    dataset = build_dataset_from_cfg(config._base_, config.others)
    if 'shuffle' in config.others:
        shuffle = config.others.shuffle
    else:
        shuffle = 'train' in config.others.subset
    if args.distributed:
        sampler = torch.utils.data.distributed.DistributedSampler(dataset, rank=args.local_rank, shuffle = shuffle)
...
def build_opti_sche_mae(base_model, config):
    opti_config = config.optimizer
    if opti_config.type == 'AdamW':
        # build optimizer with layer-wise lr decay (lrd)
        param_groups = lrd.param_groups_lrd(base_model, opti_config.kwargs.weight_decay,
            no_weight_decay_list=base_model.no_weight_decay(),
            layer_decay=opti_config.kwargs.layer_decay
        )
        optimizer = torch.optim.AdamW(param_groups, lr=opti_config.kwargs.lr)
    elif opti_config.type == 'oriAdamW':
    ...
    elif opti_config.type == 'AdamW_CLIP':
        p_wd, p_non_wd = [], []
        for n, p in base_model.named_parameters():
            ...
            if p.ndim < 2 or 'bias' in n or 'ln' in n or 'bn' in n:
                ...
                p_wd.append(p)
        optim_params = [{"params": p_wd, "weight_decay": config.optimizer.kwargs.weight_decay},
                        {"params": p_non_wd, "weight_decay": 0}]

        optimizer = torch.optim.AdamW(optim_params, lr=config.optimizer.kwargs.lr,
